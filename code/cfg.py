...
Dat=np.array(data,np.float64).reshape(num_data,fl_tim,num_fl_fea)
data=list(Dat)
...
for i in range(10):
    ...
    flmodel=FLmodel(len(train_Df),num_fl_fea,fl_tim,num_class).to(device)
    loss1=Loss
    ...
    for epoch in range(iterations):
        ...
        for param in flmodel.parameters():
            ...
            pant=obj_func(param,rol1=0.01,rol2=0.01,rol3=0.01)
            ll=l+pant
            w=param
        ll.backward()
        optimizer1.step()
